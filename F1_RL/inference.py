...
5. Prefer DEPLOY on straights and acceleration opportunities.
6. Prefer REGEN during braking or lift-off.
7. In high curvature or near lateral limit, reduce throttle, use smooth steering, avoid aggressive deploy.
8.  If done is true, output neutral safe action.

Safe neutral action:
{"throttle": 0.0,"brake": 0.0,"steering": 0.0,"regen_intensity": 0.0,"deploy_level": 0.0,"battery_status": "NEUTRAL"}

Return format:
Exactly one line JSON object, parseable by standard JSON parser.
    """
).strip()
...
async def main() -> None:
    ...
    env = await F1EnvClient.from_docker_image(IMAGE_NAME)
    grader = AgentGrader()
